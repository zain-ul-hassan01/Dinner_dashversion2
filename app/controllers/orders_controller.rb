...
class OrdersController < ApplicationController
  ...
  def index
    @orders = isAdmin? ? Order.find_orders_of_restaurant(params[:restaurant_id]) 
  ...
  def search
    if params[:status].present? && params[:status] != "Filter"
      @orders = Order.find_order_according_to_status(params[:status], @@restaurant_id)
    else
      @orders = Order.find_orders_of_restaurant(@@restaurant_id)
    end
    authorize @orders
  end
